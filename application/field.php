...
class field {
    ...
    public static function check_name($data, $field_id)
    {
        ...
        $result = db('field')->show(false)->where('api_id', '=', $data['api_id'])->where('name', '=', $data['name'])->where('method', '=', $data['method'])->where('parent_id','=',$data['parent_id'])->where('id', 'not in', [$field_id])->find();
